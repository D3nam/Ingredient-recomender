...
intNode* merge_node(intNode* h1,intNode* h2)
{
	
	intNode* merged=h1;
	intNode* traverser=h2;
	while(traverser!=NULL)
	{
		if(searchInt(merged,traverser->x)==0)
		{
			merged=insertInt(merged,traverser->x);
		}
...
intNode* avoid_node(intNode* h1,intNode* h2)
{
	intNode* merged=NULL;
	intNode* traverser=h1;
	intNode* avoider=h2;
	while(traverser!=NULL)
	{
		if(searchInt(avoider,traverser->x)==0)
		{
			merged=insertInt(merged,traverser->x);
		}
		traverser=traverser->next;
...
void related_ingredients(char source_ingredient[MAX_STR_LEN])
{
     ...
     * Implement this function
     * ********/
	int loc=ingredient_index(source_ingredient);
	int j=0;
	while(j<MAT_SIZE)
	{
		if(AdjMat[loc][j]>0)
...
intNode* related_node(char source_ingredient[MAX_STR_LEN],int notkepp)
{
    /*
     * This function prints out all the ingredients related
     * to the source ingredient. 
...
intNode *related_k_dist(intNode *h, char source_ingredient[MAX_STR_LEN], int k, int dist)
{
     ...
     * Complete this function
     *******/
	intNode* actuallist=h;
	int loc=ingredient_index(source_ingredient);
	if(searchInt(actuallist,loc)==0)
	{
		actuallist=insertInt(actuallist,loc);
	}
    if(k==0)
	{
		intNode* head=NULL;
		if(dist!=0)
		{
			head=insertInt(head,loc);
			head->next=related_node(source_ingredient,dist-1);
		}
		else
	    {
			head=related_node(source_ingredient,-8);
		}
		return head;  
	}
	if(k==1)
	{
		...
		intNode* neighbour=related_node(source_ingredient,dist-1); 
		intNode* traverser=neighbour;
		
		while(traverser!=NULL)
		{
			if(searchInt(actuallist,traverser->x)==0)
			{
				returnlist=merge_node(related_k_dist(actuallist,ingredients[traverser->x],k-1, loc+1),returnlist);
			}
			traverser=traverser->next;
		}	
		free(neighbour);
...
intNode *related_with_restrictions(char source_ingredient[MAX_STR_LEN], char avoid[MAX_STR_LEN], int k_source, int k_avoid)
{
    ...
    intNode* k_list=NULL;
	intNode* actuallist=NULL;
	k_list=related_k_dist(k_list,source_ingredient,k_source,0);
	intNode* avoid_list=NULL;
	avoid_list=related_k_dist(avoid_list,avoid,k_avoid,0);
	actuallist=avoid_node(k_list,avoid_list);
	free(k_list);
	free(avoid_list);
	return actuallist;
...
void substitute_ingredient(char recipe[10][MAX_STR_LEN], char to_change[MAX_STR_LEN])
{
   ...
   */
    ...
    ******/
	...
	{
		int loc=ingredient_index(recipe[i]);
	...
	strcpy(recipe[set],ingredients[returns]);
}
